<template>
  <b-form class="login-form" style="text-align: center" v-if="emailAccepted"
          v-on:submit.prevent="resendConfirmationEmail()">
    <div class="login-form__errors" v-if="errors.length > 0">
      <b-alert v-bind:key="index" class="login-form__errors__error " v-for="(error, index) in errors" show
               variant="danger" size="lg">
        {{error.msg}}
      </b-alert>
    </div>
    <div class="login-form__errors" v-if="messages.length > 0">
      <b-alert v-bind:key="index" class="login-form__errors__error " v-for="(message, index) in messages" show
               variant="success" size="lg">
        {{message.msg}}
      </b-alert>
    </div>
    <h2 class="login-form__header">Welcome to Note Manager</h2>
    <p> We've send you a confirmation e-mail. Click the link in your e-mail to confirm your account. If you cant find
      the
